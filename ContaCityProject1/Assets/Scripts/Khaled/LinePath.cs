...
public class LinePath  {
	...
	public Vector3 this[int i]
	{
		...
		set
		{
			nodes[i].transform.position = value;
		}
	}
	...
	public LinePath(GameObject[] nodes) {
		this.nodes = nodes;
	...
	public void calcDistances() {
		...
		for(var i = 0; i < nodes.Length - 1; i++) {
			distances[i+1] = distances[i] + Vector3.Distance(nodes[i].transform.position, nodes[i+1].transform.position);
		}
	...
	public void draw() {
		for (int i = 0; i < nodes.Length-1; i++) {
			Debug.DrawLine(nodes[i].transform.position, nodes[i+1].transform.position, Color.cyan, 0.0f, false);
		}
	}
	...
	public float getParam(Vector3 position) {
		...
		float param = this.distances[closestSegment] + getParamForSegment(position, nodes[closestSegment].transform.position, nodes[closestSegment+1].transform.position);
	...
	public int getClosestSegment(Vector3 position) {
		...
		float closestDist = distToSegment(position, nodes[0].transform.position, nodes[1].transform.position);
		int closestSegment = 0;
		
		for(int i = 1; i < nodes.Length - 1; i++) {
			float dist = distToSegment(position, nodes[i].transform.position, nodes[i+1].transform.position);
	...
	public Vector3 getPosition(float param, bool pathLoop = false) {
		...
		float t = (param - distances[i]) / Vector3.Distance(nodes[i].transform.position, nodes[i+1].transform.position);
		
		/* Get the position of the param */
		return Vector3.Lerp(nodes[i].transform.position, nodes[i+1].transform.position, t);
	}
	...
	public void removeNode(int i ) {
		GameObject[] newNodes = new GameObject[nodes.Length - 1];
